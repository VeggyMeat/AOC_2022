...
while True:
    ...
    if item == end:
        ...
        if (item[0], item[1] - 1) not in next_blizzards and (item[1] - 1 > 0 or (item[0], item[1] - 1) == end):
            if (item[0], item[1] - 1) not in next_search:
                search.append((item[0], item[1] - 1))
                next_search.add((item[0], item[1] - 1))
...
while True:
    ...
    if item == end:
        ...
        if (item[0] + 1, item[1]) not in next_blizzards and width - 1 > item[0] + 1 and height - 1 > item[1] > 0:
            if (item[0] + 1, item[1]) not in next_search:
                search.append((item[0] + 1, item[1]))
                next_search.add((item[0] + 1, item[1]))
        if (item[0] - 1, item[1]) not in next_blizzards and item[0] - 1 > 0 and height - 1 > item[1] > 0:
            if (item[0] - 1, item[1]) not in next_search:
                search.append((item[0] - 1, item[1]))
                next_search.add((item[0] - 1, item[1]))
        if (item[0], item[1] + 1) not in next_blizzards and (height - 1 > item[1] + 1 or (item[0], item[1] + 1) == end):
